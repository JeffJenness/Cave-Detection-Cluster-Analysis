...
'  ReturnAppFolder - RETURNS FOLDER CONTAINING DLL
...
'  ReturnGraphicsBySelectionFromLayout - GIVEN A MAP, RETURNS THE SELECTED ELEMENTS FROM THE LAYOUT. USED TO GENERATE TRANSPARENT FEATURE CLASSES AROUND REGION.
...
'  ReturnMXDFolder - RETURNS FOLDER CONTAINING THIS MXD
...
'  ReturnWorkspaceFactoryType - GIVEN Class ID string (a GUID), returns the type of workspace factory (Access, Excel, SDE, Shapefile, etc.)
'  ReturnWorkspaceTypeName - GIVEN WORKSPACE, RETURNS "Local Database Workspace", "Remote Database Workspace" OR "Unknown Workspace Type"
...
Public Sub AddTableToMxDoc(pTable As ITable, pMxDoc As IMxDocument, pApp As IApplication, Optional booOpenTable As Boolean = True)
...
'  Dim pWSFact As IWorkspaceFactory
'  Set pWSFact = New FileGDBWorkspaceFactory
'  Set pWS = pWSFact.OpenFromFile("E:\arcGIS_stuff\consultation\Springs_Stewardship_Institute\Range_Maps\Map_Boxes.gdb", 0)
'  Dim strName As String
...
Public Sub MakeFClassBorderAroundJennessentCompassRose(pMxDoc As IMxDocument)
  ...
  Dim pWSFact As IWorkspaceFactory
  Set pWSFact = New FileGDBWorkspaceFactory
  Set pWS = pWSFact.OpenFromFile("E:\arcGIS_stuff\consultation\Ants_Gary_Beverly\Temp\Temp.gdb", 0)
  Dim strName As String
  Dim pWS2 As IWorkspace2
  Set pWS2 = pWS
  Dim pEnv As IEnvelope
...
'
'  Debug.Print "-----------------------------"
'  strPath = "E:\arcGIS_stuff\Teaching\Semester_C_2014\Data\Additional_Data\CocRoadStatus_20120425.mdb\ CocRoadStatus_20120425"
'
...
' SAMPLE CODE
'  Dim strFileName As String
'  strFileName = "E:\arcGIS_stuff\consultation\Jut_Wynne\Analysis_Files\October_16_2015_Outputs\Mojave_Day_Raster_Data_Oct_16_2015.csv"
'
'  Dim pWS As IFeatureWorkspace
'  Dim pWSFact As IWorkspaceFactory
'  Set pWSFact = New FileGDBWorkspaceFactory
'  Set pWS = pWSFact.OpenFromFile("E:\arcGIS_stuff\consultation\Jut_Wynne\Analysis_Files\October_16_2015_Outputs\test.gdb", 0)
'
...
Public Function CreateFieldAttributeIndex(pTable As ITable, strFieldName As String, _
  ...
  If pIndex Is Nothing Then
    ...
    If pField Is Nothing Then
      ...
      strFailReason = "No Field with name '" & strFieldName & "'"
    ...
    Else
      ...
      Debug.Print "  --> Built Index for '" & strFieldName & "'"
...
'    Call InvoiceCreator.MyGeneralOperations.ExtensionNotRegisteredMessage( _
...
'
'  strName1 = "E:\arcGIS_stuff\general_tools\Registration\MyGeometricOperations.bas"
'  strName2 = "E:\arcGIS_stuff\consultation\USGS\Cross_Section\Code\MyGeometricOperations.bas"
...
Public Function ReturnEmptyFClassWithSameSchema(pFClass As IFeatureClass, pWS_NothingForInMemory As IWorkspace, _
    varFieldIndexArray() As Variant, strName As String, booHasFields As Boolean, Optional booForceShapeType As Boolean = False, _
    Optional lngForceShapeType As esriShapeType = esriShapeTypeLast, Optional pForceSpatialReference As ISpatialReference = Nothing) _
    As IFeatureClass
  
  ' varFieldIndexArray WILL HAVE 4 COLUMNS AND ANY NUMBER OR ROWS.
  ' COLUMN 0 = SOURCE FIELD NAME
  ' COLUMN 2 = SOURCE FIELD INDEX
  ' COLUMN 3 = NEW FIELD NAME
  ' COLUMN 4 = NEW FIELD INDEX
  ...
  strWSID = MyGeneralOperations.ReturnWorkspaceFactoryType(pWS_NothingForInMemory.WorkspaceFactory.GetClassID)
  
  If pWS_NothingForInMemory Is Nothing Then
    booIsInMem = True
  Else
    booIsInMem = False
    booIsShapefile = strWSID = "Esri Shapefile Workspace Factory" ' TypeOf pWS_NothingForInMemory.WorkspaceFactory Is ShapefileWorkspaceFactory
    booIsAccess = strWSID = "Esri Access Workspace Factory" ' TypeOf pWS_NothingForInMemory.WorkspaceFactory Is ShapefileWorkspaceFactory
    booIsFGDB = strWSID = "File GeoDatabase Workspace Factory" ' TypeOf pWS_NothingForInMemory.WorkspaceFactory Is FileGDBWorkspaceFactory
  End If
  ...
  Dim lngShapeType As esriShapeType
  If booForceShapeType Then
    lngShapeType = lngForceShapeType
  Else
    lngShapeType = pGeomDef.GeometryType
  End If
  
  Dim pNewSpRef As ISpatialReference
  If pForceSpatialReference Is Nothing Then
    Set pNewSpRef = pGeoDataset.SpatialReference
  Else
    Set pNewSpRef = pForceSpatialReference
  End If
  
  Dim pNewFClass As IFeatureClass
  If booIsInMem Then
    Set pNewFClass = MyGeneralOperations.CreateInMemoryFeatureClass_Empty(pNewFieldArray, strName, pNewSpRef, _
        lngShapeType, pGeomDef.HasM, pGeomDef.HasZ)
  ElseIf booIsFGDB Or booIsAccess Then
    Set pNewFClass = MyGeneralOperations.CreateGDBFeatureClass2(pWS_NothingForInMemory, strName, esriFTSimple, pNewSpRef, _
        lngShapeType, pNewFieldArray, , , , False, lngCategory, pGeoDataset.Extent, , pGeomDef.HasZ, pGeomDef.HasM)
  ElseIf booIsShapefile Then
    Set pNewFClass = MyGeneralOperations.CreateShapefileFeatureClass2(pWS_NothingForInMemory.PathName, strName, _
        pNewSpRef, lngShapeType, pNewFieldArray, False, pGeomDef.HasZ, pGeomDef.HasM)
  Else
...
Public Function ExportToCSV(pFLayerOrStandaloneTable As IUnknown, strFileName As String, _
    ...
    Optional pApp As IApplication = Nothing) As String
  ...
  If Not (TypeOf pFLayerOrStandaloneTable Is IStandaloneTable Or _
          TypeOf pFLayerOrStandaloneTable Is IFeatureLayer) Then
    booSucceeded = False
  ...
  Else
    ...
    Do Until pFeature Is Nothing
      ...
      For lngIndex = 0 To UBound(varFieldIndexes, 2)
        lngFieldIndex = varFieldIndexes(1, lngIndex)
        lngFieldType = varFieldIndexes(2, lngIndex)
        varVal = pFeature.Value(lngFieldIndex)
        If IsNull(varVal) Then
          strReport = strReport & ","
        Else
          If lngFieldType = esriFieldTypeString Or lngFieldType = esriFieldTypeXML Then
...
Public Function ReturnMXDFolder(booAddTrailingSlash As Boolean, pApp As IApplication) As String

  ReturnMXDFolder = InvoiceCreator.aml_func_mod.ReturnDir3(InvoiceCreator.aml_func_mod.GetMxDocPath(pApp), True)

End Function

Public Function ReturnAppFolder(booAddTrailingSlash As Boolean) As String
  ReturnAppFolder = App.Path
  If booAddTrailingSlash Then
    If Right(ReturnAppFolder, 1) <> "\" Then
      ReturnAppFolder = ReturnAppFolder & "\"
    End If
  Else
    If Right(ReturnAppFolder, 1) = "\" Then
      ReturnAppFolder = Left(ReturnAppFolder, Len(ReturnAppFolder) - 1)
    End If
  End If

End Function

Public Function ReturnGraphicsBySelectionFromLayout(ByRef pMxDoc As IMxDocument, _
      Optional AsElements As Boolean) As IArray

  Dim pGraphicsContainerSelect As IGraphicsContainerSelect
  
  Set pGraphicsContainerSelect = pMxDoc.PageLayout
  Dim pEnumElement As IEnumElement
  Set pEnumElement = pGraphicsContainerSelect.SelectedElements
  pEnumElement.Reset
  ...
  Wend
  Set ReturnGraphicsBySelectionFromLayout = pArray
...
Public Function CreateOrReplaceFieldAttributeIndex(pTable As ITable, strFieldName As String, _
    Optional strFailReason As String) As Boolean
  
  CreateOrReplaceFieldAttributeIndex = True
  strFailReason = ""
  Dim pIndexes As IIndexes
  Dim pEnumIndex As IEnumIndex
  Dim pFields As IFields
  ...
  If pTable.FindField(strFieldName) = -1 Then
    CreateOrReplaceFieldAttributeIndex = False
    strFailReason = "No Field with name '" & strFieldName & "'"
  Else
    
    Set pSchemaLock = pTable
    ...
    If Not pIndex Is Nothing Then pTable.DeleteIndex pIndex
...
Public Function CreateFieldAttributeIndex2(pTable As ITable, strFieldName As String, _
    Optional strFailReason As String, Optional booRebuildIfPresent As Boolean = False) As Boolean
  
  CreateFieldAttributeIndex2 = True
  strFailReason = ""
  Dim pIndexes As IIndexes
  Dim pEnumIndex As IEnumIndex
  Dim pFields As IFields
  ...
  Dim pIndex As IIndex
  Dim pIndexEdit As IIndexEdit
  Dim pIndexesEdit As IIndexesEdit
  Dim strIndexName As String
  
  If pTable.FindField(strFieldName) = -1 Then
    CreateFieldAttributeIndex2 = False
    strFailReason = "No Field with name '" & strFieldName & "'"
  Else
    ...
    If pIndex Is Nothing Then
      ...
      End If
    Else
      If booRebuildIfPresent Then
        strIndexName = pIndex.Name
        Set pIndexesEdit = pIndexes
        pIndexesEdit.DeleteIndex pIndex
        ...
        Else
          ...
          pIndexEdit.IsAscending = False
          pIndexEdit.IsUnique = False
          pIndexEdit.Name = strIndexName
          
          ' Add the index to the feature class.
          pTable.AddIndex pIndex
          
          Debug.Print "  --> ReBuilt Index for '" & strIndexName & "'"
...
Public Sub SetMapExtentToEnv(pMap As IMap, pEnv As IEnvelope)
  ...
  Dim pPolygon As IPolygon
  If Not CompareSpatialReferences(pSpRef, pEnv.SpatialReference) Then
    Set pPolygon = MyGeometricOperations.EnvelopeToPolygon(pEnv)
    pPolygon.Project pSpRef
...
Public Sub ImportASCIIPoints()
  ...
  strFile = MyGeneralOperations.ReadTextFile("E:\arcGIS_stuff\consultation\Jut_Wynne\2017_09_28_Data\Predictions_Night.csv")
  
  Debug.Print "Splitting File..."
  Dim strSplit() As String
  Dim strLine As String
  Dim lngIndex As Long
  Dim strLineSplit() As String
  ...
  psbar.ShowProgressBar "Traversing file...", 0, UBound(strSplit) + 1, 1, True
  pProg.position = 0
  
  Debug.Print "Going through lines..."
  For lngIndex = 0 To UBound(strSplit)
    pProg.Step
    If lngIndex Mod 1000 = 0 Then
      DoEvents
    End If
    strLine = strSplit(lngIndex)
  Next lngIndex
